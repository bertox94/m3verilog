...
`endif
    ...
    localparam CLK_PERIOD_BAUD      = 8640000;
    localparam CLK_PERIOD_BAUD_DS   = CLK_PERIOD_BAUD/2;
...
//    localparam CLK_DIV_BAUD    = 10416;     // Should be sum above, but measurement in sim results in this value; when 230400
    ...
    //always #(CLK_PERIOD_BAUD_DS/2)  clk_baud_ds <= ~clk_baud_ds;

   
    logic baud_c = 0;
    always #(CLK_PERIOD_BAUD_DS/2) begin  
        if(delay_baud == 0) begin      
            clk_baud_ds <= ~clk_baud_ds;
            baud_c ++;
        end else delay_baud = 0;
    end
    always @ (posedge baud_c) clk_baud <= ~clk_baud;
...
`endif
    ...
    reg[8:1]    uart_tx_pkt;
    reg[8:1]    uart_rx_pkt;
    reg[13:1]   uart_h_tx_pkt;
    reg[13:1]   uart_h_rx_pkt;
    reg         out_bit_tx;
    reg         in_bit_rx;
    reg         fixed_error;
    reg         unfixable_error;
    ...
    initial begin
        ...
        @(posedge clk_baud) reset = 1;
        @(posedge clk_baud) reset = 0;
      
    end

    int pos1 = 143;//-1;
    int pos2 = 202;//-2
    always @(negedge clk_baud_ds) begin
        if(count_r == pos1 ) begin
            in_bit_rx = ~ out_bit_tx;
        end else if(count_r == pos2 || count_r == pos2 + 1) begin
             in_bit_rx = ~ out_bit_tx;
        end else begin
             in_bit_rx = out_bit_tx;
        end
